...
prepare_inputs() {
  ...
  "$PYTHON_BIN" "$EVAL_DIR/summarize_full_scene_gate_r01_v24.py" --prepare-protocol
}

verify_protocol() {
  "$PYTHON_BIN" "$EVAL_DIR/summarize_full_scene_gate_r01_v24.py" >/dev/null
}
...
run_one() {
  ...
  "$PYTHON_BIN" "$EVAL_DIR/evaluate_multi_agent.py" \
    "$EXPERIMENT_ROOT/$arm/R01/drama_evaluator_logic_quality_input/full_60_episodes.txt" \
    --output-dir "$EXPERIMENT_ROOT/$arm/R01/drama_evaluations_logic_quality_qwen38_v24/full_60_episodes" \
...
case "$ACTION" in
  ...
  dry-run)
    prepare_inputs
    for arm in "${arms[@]}"; do run_one "$arm" --dry-run; done
    ;;
  status)
    verify_protocol
    "$PYTHON_BIN" "$EVAL_DIR/summarize_full_scene_gate_r01_v24.py"
    ;;
  report)
    verify_protocol
    "$PYTHON_BIN" "$EVAL_DIR/summarize_full_scene_gate_r01_v24.py" --write
    ;;
  run)
    prepare_inputs
    ...
    flock -n 9 || { echo 'R01 v2.4 评测已经在运行' >&2; exit 2; }
    pids=()
    for arm in "${arms[@]}"; do
      (set -o pipefail; run_one "$arm" 2>&1 | tee -a "$LOG_ROOT/full_scene_gate_logic_quality_v24_${arm}.log") &
      pid="$!"; pids+=("$pid")
      echo "$arm PID=$pid log=$LOG_ROOT/full_scene_gate_logic_quality_v24_${arm}.log"
    done
    failed=0
    ...
    "$PYTHON_BIN" "$EVAL_DIR/summarize_full_scene_gate_r01_v24.py"
    [[ "$failed" -eq 0 ]] || { echo '部分 v2.4 评测未完成；重跑同一命令续跑。' >&2; exit 1; }
    "$PYTHON_BIN" "$EVAL_DIR/summarize_full_scene_gate_r01_v24.py" --write
    ;;
  *)
    echo '用法：run_full_scene_gate_r01_v24.sh {prepare|dry-run|run|status|report}' >&2
    exit 2
    ;;
esac
